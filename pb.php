						...
						if($sr)
						{
							...
							foreach($records as $account)
							{
								if(!empty($account['samaccountname'][0]) && !empty($account['givenname'][0]) && !empty($account['sn'][0]))
								{
									...
									$s_phone_internal = @$account['ipphone'][0];
									$s_phone_mobile = @$account['mobile'][0];
									$s_mail = @$account['postofficebox'][0];
									$s_mosmail = @$account['mail'][0];
									$s_office = @$account['physicaldeliveryofficename'][0];
						...
						if($sr)
						{
							...
							foreach($records as &$account)
							{
								if(!empty($account['samaccountname'][0]))
								{
									...
									if($s_disabled && $db->select(rpv('SELECT m.`id`, m.`samname`, m.`fname`, m.`lname`, m.`dep`, m.`org`, m.`office`, m.`pos`, m.`pint`, m.`pcell`, m.`mail`, m.`mosmail`, m.`photo`, m.`map`, m.`x`, m.`y`, m.`visible` FROM `@contacts` AS m WHERE m.`samname` = ! AND m.`visible` = 1 LIMIT 1', $s_login)))
									{
										$id = $db->data[0][0];
										$db->data[0][14] = 0;
										$data[] = $db->data[0];
